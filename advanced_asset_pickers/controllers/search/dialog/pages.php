...
class Pages extends Controller
{
    ...
    public function __construct()
    {
        ...
        $this->searchBar = true;
        $this->attributeHandle = $_REQUEST['handle'];
        $this->attributeKeyID = $_REQUEST['akID'];
    ...
    public function search()
    {
        ...
        if(method_exists("Application\Src\AdvancedAttributeFilters",$handleName)){
            \Application\Src\AdvancedAttributeFilters::$handleName($this,$this->pageList);
        }else{

            $options = MultipagePickerAttributeTypeValue::getOptions($this->attributeKeyID);
            if($options['path']){
                $this->pageList->filterByPath($options['path']);
            }
            
            if($options['parentId']){
                $this->pageList->filterByParentID(intval($options['parentID']));
            }

            if($options['typeHandle']){
                $this->pageList->filterByCollectionTypeHandle($options['typeHandle']);
            }
        }
       

        $ilr = new PageSearchResult($columns, $this->pageList,URL::to($this->getSearchURL()) , $this->fields);
        $this->result = $ilr;
    }

    public function getSearchURL(){
        if($this->attributeHandle){
            return '/advanced_asset_pickers/page_search/submit/'.$this->attributeHandle.'/'.$this->attributeKeyID;
        }else{
            return '/advanced_asset_pickers/page_search/submit';
        }
    ...
    public function submit($handle='',$key='')
    {
        $this->attributeHandle = $handle;
        $this->attributeKeyID = $key;
        $this->search();
        $result = $this->result;
    ...
    public function getPagesJSON() {
        ...
        if ($h->canRead()) {
            ...
            $pages = array_filter(array_map(function($item){
                $c = ConcretePage::getByID(intval($item));
